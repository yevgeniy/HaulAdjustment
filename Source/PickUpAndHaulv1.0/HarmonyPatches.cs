...
namespace PickUpAndHaul
{
    ...
    static class HarmonyPatches
    {
        static HarmonyPatches()
        {
            ...
            harmony.Patch(original: AccessTools.Method(typeof(StoreUtility), "TryFindBestBetterNonSlotGroupStorageFor"),
                prefix: new HarmonyMethod(typeof(HarmonyPatches), nameof(LookForSupremeStoreSpace)));

            Verse.Log.Message("PickUpAndHaul v0.1.0.5¼ welcomes you to RimWorld with pointless logspam.", true);
        }

        private static bool TryFindBestBetterNonSlotGroupStorageFor(ref bool __result,  Thing t, Pawn carrier, Map map, StoragePriority currentPriority, Faction faction, out IHaulDestination haulDestination, bool acceptSamePriority, bool requiresDestReservation)
        {
            haulDestination = null;

            var constructableHaulDestination = ConstructablesMap.GetLocationOfMatchingConstructable(carrier, t);

            if (constructableHaulDestination != null)
            {

                haulDestination = constructableHaulDestination;
